...
public class ChiTietSPRepository implements IChiTietSPRepository {
    ...
    final String SQL_SELECT_ALL = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,IdCA,IdPC,SoLuongTon,GiaNhap,GiaBan,MoTa,QrCode FROM dbo.ChitietSP WHERE SoLuongTon > 0";
    final String SQL_SELECT_BY_MA = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,IdCA,IdPC,SoLuongTon,GiaNhap,GiaBan,MoTa,QrCode FROM dbo.ChitietSP WHERE Ma = ?";
    final String SQL_SELECT_BY_SL = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,IdCA,IdPC,SoLuongTon,GiaNhap,GiaBan,MoTa,QrCode FROM dbo.ChitietSP WHERE SoLuongTon = 0";
    final String SQL_SELECT_BY_TEN = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,IdCA,IdPC,SoLuongTon,GiaNhap,GiaBan,MoTa,QrCode FROM dbo.ChitietSP WHERE SoLuongTon > 0 AND ten LIKE ?";
    final String SQL_INSERT = "INSERT INTO dbo.ChitietSP\n"
            + "( Ma, Ten, IdNsx, IdMauSac, IdDMuc, IdKC, IdCL, IdTH,IdCA,IdPC, MoTa, SoLuongTon, GiaNhap, GiaBan, QrCode)\n"
            + "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    final String SQL_UPDATE = "UPDATE dbo.ChitietSP SET Ma = ?, Ten = ?, IdNsx = ?, IdMauSac = ?, IdDMuc = ?, IdKC = ?, IdCL = ?, IdTH = ?,IdCA = ?,IdPC = ?, MoTa = ?, SoLuongTon = ?, GiaNhap = ?, GiaBan = ? WHERE Ma = ?";
...
//            Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,SoLuongTon,GiaNhap,GiaBan,MoTa,QrCode
    ...
    public int insert(ChiTietSP x) {
        return DBConnection.ExcuteQuery(SQL_INSERT, x.getMa(), x.getTen(), x.getIdnsx(), x.getIdmausac(), x.getIddanhmuc(), x.getIdkc(), x.getIdcl(), x.getIdth(),x.getIdca(),x.getIdpc(), x.getMota(), x.getSoluongton(), x.getGianhap(), x.getGiaban(), x.getQrcode());
    }

    @Override
    public int update(ChiTietSP x, String Ma) {
        return DBConnection.ExcuteQuery(SQL_UPDATE, x.getMa(), x.getTen(), x.getIdnsx(), x.getIdmausac(), x.getIddanhmuc(), x.getIdkc(), x.getIdcl(), x.getIdth(),x.getIdca(),x.getIdpc(), x.getMota(), x.getSoluongton(), x.getGianhap(), x.getGiaban(), Ma);
    }
