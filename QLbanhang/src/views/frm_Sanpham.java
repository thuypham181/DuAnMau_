...
//        listctsp = iChiTietSPServices.getAll();
    ...
    public void initcbo() {
        ...
        cbo_thuonghieu1.setModel(new DefaultComboBoxModel(listth.toArray()));
         ...
         List<CoAo> listca = iCoAoServices.getAll();
        cbo_CoAo.setModel(new DefaultComboBoxModel(listca.toArray()));
        
         List<PhongCach> listpc = iPhongCachServices.getAll();
        cbo_PhongCach.setModel(new DefaultComboBoxModel(listpc.toArray()));
        
         List<KhuyenmaiViewmodel> listkm = iKhuyenmaiService.GetALL();
        cbo_KM.setModel(new DefaultComboBoxModel(listkm.toArray()));
    ...
    private int getindexca(ChiTietSPViewModel x) {
        List<CoAo> lst = iCoAoServices.getAll();
        int index = -1;
        for (int i = 0; i < lst.size(); i++) {
            if (lst.get(i).getId() == x.getCoAo().getId()) {
                index = i;
            }
        }
        return index;
    }
      private int getindexpc(ChiTietSPViewModel x) {
        List<PhongCach> lst = iPhongCachServices.getAll();
        int index = -1;
        for (int i = 0; i < lst.size(); i++) {
            if (lst.get(i).getId() == x.getPhongCach().getId()) {
                index = i;
            }
        }
        return index;
    ...
    private String zenma() {
        int size = iChiTietSPServices.GetAll().size() + 1;
        return "SP" + size;
    }
    ...
    private ChiTietSPViewModel getdadtafrom() {
        ...
        if (txt_ten.getText().length() >= 50) {
            JOptionPane.showMessageDialog(this, "Tên sản phẩm không được quá dài");
            return null;
        }
// Số lượng tồn
        ...
        if (Integer.valueOf(txt_soluongton.getText()) <= -1) {
...
// giá bán
        ...
        CoAo coao = (CoAo) cbo_CoAo.getSelectedItem();
        PhongCach pc = (PhongCach) cbo_PhongCach.getSelectedItem();
        double gianhap;
        double giaban;
        int soluong;
        try {
        ...
        giaban = Double.parseDouble(txt_giaban.getText());
        soluong = Integer.parseInt(txt_soluongton.getText());
        ChiTietSPViewModel ctsp = new ChiTietSPViewModel(ma, txt_ten.getText(), nsx, mausac, danhmuc, kichco, chatlieu, thuonghieu,null,coao,pc, soluong, gianhap, giaban, mota, zenbarcode());
        return ctsp;
    }
    ...
    private ChiTietSPViewModel getdataTB(int row) {
        ...
        CoAo ca= (CoAo) tbl_sp.getValueAt(row, 9);
        PhongCach pc = (PhongCach) tbl_sp.getValueAt(row, 10);
        int soluong = (int) tbl_sp.getValueAt(row, 11);
        double gianhap = (double) tbl_sp.getValueAt(row, 12);
        double giaban = (double) tbl_sp.getValueAt(row, 13);
        String mota = tbl_sp.getValueAt(row, 14).toString();
        String barcode = tbl_sp.getValueAt(row, 15).toString();
        return new ChiTietSPViewModel(ma, ten, nsx, ms, dmsp, kc, cl, th, km,ca,pc, soluong, gianhap, giaban, mota, barcode);
    }
    ...
    private void initComponents() {
        ...
        btn_xuatExcel = new swing.MyButton();
        btnXoa = new javax.swing.JButton();
        jLabel9 = new javax.swing.JLabel();
        cbo_CoAo = new javax.swing.JComboBox<>();
        jLabel16 = new javax.swing.JLabel();
        cbo_PhongCach = new javax.swing.JComboBox<>();
        ...
        jLabel2.setBounds(30, 70, 210, 20);

        jLabel3.setText("Nhà sản xuất");
        panelBorder1.add(jLabel3);
        jLabel3.setBounds(30, 130, 210, 20);
        ...
        cbo_nsx.setBounds(30, 160, 210, 40);
        ...
        cbo_mausac.setBounds(30, 220, 210, 40);
        ...
        txt_ten.setBounds(30, 90, 210, 40);

        btn_xuatExcel.setBackground(new java.awt.Color(125, 224, 237));
        btn_xuatExcel.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/floppy-disk.png"))); // NOI18N
        btn_xuatExcel.setText("xuất Ecxel");
        btn_xuatExcel.setFont(new java.awt.Font("Tahoma", 1, 12)); // NOI18N
        btn_xuatExcel.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_xuatExcelActionPerformed(evt);
            }
        });
        panelBorder1.add(btn_xuatExcel);
        btn_xuatExcel.setBounds(810, 220, 140, 40);
        ...
        panelBorder1.add(btnXoa);
        btnXoa.setBounds(810, 270, 140, 36);

        jLabel9.setText("Màu sắc");
        panelBorder1.add(jLabel9);
        jLabel9.setBounds(30, 200, 210, 20);

        cbo_CoAo.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
        cbo_CoAo.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 204, 204), 2));
        panelBorder1.add(cbo_CoAo);
        cbo_CoAo.setBounds(20, 300, 210, 40);

        jLabel16.setText("Phong Cách");
        panelBorder1.add(jLabel16);
        jLabel16.setBounds(280, 290, 210, 20);

        cbo_PhongCach.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
        cbo_PhongCach.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 204, 204), 2));
        panelBorder1.add(cbo_PhongCach);
        cbo_PhongCach.setBounds(270, 310, 210, 40);

        jLabel17.setText("Cổ Áo");
        panelBorder1.add(jLabel17);
        jLabel17.setBounds(30, 270, 210, 20);

        cbo_KM.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
        cbo_KM.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 204, 204), 2));
        panelBorder1.add(cbo_KM);
        cbo_KM.setBounds(520, 320, 210, 40);

        panelGradiente1.add(panelBorder1);
        panelBorder1.setBounds(8, 0, 990, 360);
        ...
        panelBorder2.setBounds(10, 360, 990, 300);
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                .addComponent(panelGradiente1, javax.swing.GroupLayout.PREFERRED_SIZE, 1018, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(0, 0, Short.MAX_VALUE))
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(panelGradiente1, javax.swing.GroupLayout.DEFAULT_SIZE, 699, Short.MAX_VALUE)
        );
...
//            xuatbarcode(x);
    ...
    private void tbl_spMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_spMouseClicked
        ...
        cbo_CoAo.setSelectedIndex(getindexca(x));
        cbo_PhongCach.setSelectedIndex(getindexpc(x));
    ...
    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        ...
        if (chon == JOptionPane.YES_OPTION) {
            String idStr = (String) this.tbl_sp.getValueAt(row, 0);
            this.iChiTietSPServices.Delete(idStr);
            loadData(iChiTietSPServices.getAll());
    ...
    private void btn_xoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_xoaActionPerformed
        ...
        //        xuatbarcode(x);
        String data = x.getQrcode();
        String path = "C:\\Users\\ADMIN\\Downloads\\duan1\\duan1\\DuAn1" + x.getTen() + ".png";
        Map<EncodeHintType, ErrorCorrectionLevel> hashMap = new HashMap<EncodeHintType, ErrorCorrectionLevel>();
        hashMap.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.L);
        generateQRcode(data, path, hashMap, 200, 200);
    ...
    private swing.MyButton btn_xuatExcel;
    private javax.swing.JComboBox<String> cbo_CoAo;
    private javax.swing.JComboBox<String> cbo_KM;
    private javax.swing.JComboBox<String> cbo_PhongCach;
    ...
    private void innitTable() {
        ...
        defaultTableModel.addColumn("Khuyến mãi");
         defaultTableModel.addColumn("Cổ Áo");
        defaultTableModel.addColumn("Phong Cách");
