...
public class ChiTietSPServices implements IChiTietSPServices {
    ...
    private ICoAoRepository icoao = new CoAoRepositoryimpl();
    private IPhongCachRepository iphongcach = new PhongCachRepositoryimpl();
    ...
    public List<ChiTietSPViewModel> getAll() {
        ...
        for (ChiTietSP x : list) {
            lst.add(new ChiTietSPViewModel(
                    ...
                    icoao.getbyid(x.getIdca()),
                    iphongcach.getbyid(x.getIdpc()),
                    x.getSoluongton(),
                    x.getGianhap(),
                    x.getGiaban(),
                    x.getMota(),
    ...
    public boolean Add(ChiTietSPViewModel x) {
        ...
        ChiTietSP ctsp = new ChiTietSP(x.getMa(), x.getTen(), x.getNsx().getId(), x.getMausac().getId(), x.getDanhmuc().getId(), x.getKichco().getId(), x.getChatlieu().getId(), x.getThuonghieu().getId(),1,x.getCoAo().getId(),x.getPhongCach().getId(), x.getSoluongton(), x.getGianhap(), x.getGiaban(), x.getMota(), x.getQrcode());
        try {
    ...
    public boolean Update(String ma, ChiTietSPViewModel x) {
        ChiTietSP ctsp = new ChiTietSP(x.getMa(), x.getTen(), x.getNsx().getId(), x.getMausac().getId(), x.getDanhmuc().getId(), x.getKichco().getId(), x.getChatlieu().getId(), x.getThuonghieu().getId(),0,x.getCoAo().getId(),x.getPhongCach().getId(), x.getSoluongton(), x.getGianhap(), x.getGiaban(), x.getMota(), x.getQrcode());
    ...
    public int Delete(String ma) {
        return chiTietSPRepository.delete(ma);
    }
